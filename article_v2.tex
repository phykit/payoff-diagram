...
\lhead{\footnotesize\itshape One Trade, Twelve Honest Pictures}
...
\begin{center}
{\LARGE\bfseries One Trade, Twelve Honest Pictures}\\[6pt]
{\large\itshape The payoff diagram is not a risk display}\\[10pt]
{\large Simon Kittson}\\[14pt]
\end{center}

\begin{quote}\small\itshape
The payoff diagram is not lying to you. It is answering a question nobody at the point of sale is asking. The hockey stick is the most widely deployed piece of risk communication in retail finance, and it describes one second of a position's life and specifies no probability measure: every terminal price is drawn, none is weighted. It is the right picture for terminal contract geometry, and it suppresses the three things a holder needs, time, volatility and a measure. This article proposes a display standard for intermediate-date option risk that restores all three: finite-displacement \PnL{} curves, volatility scenarios side by side, and a stated conditional measure on every panel, with its assumptions printed on its face. The model behind the numbers is one illustrative implementation; the standard is the contribution, and the test that would settle whether it works is specified.
\end{quote}

\vspace{6pt}
\noindent\fbox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{\small
\textbf{What this article contributes.} i) A formal separation of terminal payoff from mark-to-market \PnL{}. ii) A display standard: a fixed small-multiple grid indexed by elapsed time and implied-vol regime, every element labelled scenario, model-implied, historical or market-implied. iii) One illustrative implementation of the measure: a conditional spot distribution per cell with spot--vol correlation, implied and realised vol distinguished, calibrated to Apple and re-estimated on six other underlyings in three asset classes. iv) Cell-level $P(\Pi>0)$, $E[\Pi]$ and the 5\% \PnL{} quantile. v) A frequency-framed retail mode with a one-row point-of-sale default. vi) A robustness table and a limitations section saying which parts of the display are arithmetic and which are model. vii) A powered user study, specified and not yet run.}}
\vspace{6pt}

\section{The most successful wrong chart in finance}

\noindent Most retail options platforms, textbooks and broker education pages lead with the same picture: profit or loss against terminal share price, and a kinked line. It is probably the most viewed chart in finance, and retail education still teaches it as \emph{the} risk visual whilst conceding that most positions are closed before expiry.

The diagram is very good at what it is for: comparing structures at expiry, locating break-evens and maximum loss, teaching the shape of optionality. The charge is not that it is wrong but that a terminal-payoff display has been institutionalised as a \emph{risk} display, and the charge is not new. Israelov and Nielsen (2014) listed among their eight myths of covered-call investing the belief that ``risk exposure can be expressed in a payoff diagram'', and Wystup (2019), in these pages, put the confusion of payoff diagrams with \PnL{} diagrams first among his common misconceptions. The diagram answers one question, what the position is worth at expiry; what it is worth \emph{now}, what happens if implied volatility moves, and how \emph{likely} any outcome is lie outside its vocabulary.
...
\textbf{It specifies no measure.} The horizontal axis extends impartially in both directions, as though every terminal price were equally worth contemplating; the geometry of the payoff is presented whilst the distribution it will be integrated against, under whichever measure the reader has in mind, is suppressed. A risk display without a measure is not a simplification; it is half a pricing kernel masquerading as the whole.

Neither omission mattered when the audience was structurers who carried the missing surfaces in their heads. In July 2026 zero-days-to-expiry contracts reached a record 66.2\% of SPX options volume by Cboe's own reporting (Cboe, 2026a), with retail activity rebounding alongside (Cboe, 2026c), and at 0DTE tenors the intraday path and the vol dynamics \emph{are} the trade. The consequences are measured. Brown (2022) asked in these pages whether individual investors should trade options at all: they underperform (Bauer, Cosemans and Eichholtz, 2009), the modern flow is short-dated and near the money (Bogousslavsky and Muravyev, 2024), and de Silva, Smith and So (2026) show retail traders buying options ahead of earnings in anticipation of volatility that is already in the price, and losing systematically when it collapses: a population-level loss in exactly the dimension the payoff diagram hides. Eaton, Green, Roseman and Wu (2026) add the twist that retail flow now measurably distorts the implied volatility surface itself. A cohort large enough to move the parameter is shown, at the point of sale, a picture in which it does not exist.
...
Frictions deserve numbers. The curves are mid-market; an unwind crosses the bid--ask twice on each leg. Table~2 prices the round trip under stated spread assumptions: a 2\% spread costs 4\% of the floor, a mid-cap name at 10\% a fifth of it, and a retail-sized multi-leg order meeting a spread of the order of 20\% loses nearly half the windfall. Figure~2 draws the 2\% and 10\% cases and the floor survives both, at $+4.0$ and $+3.3$. The smile is likewise suppressed: a flat 45\% repricing is a scenario, not a calibration. Both effects bias the pictures optimistic, and both are invisible on the standard diagram. None of this is news to anyone who marks a book; the point is that the \emph{display} at the point of sale cannot say it. The remedy proposed in Section~4 is a grid of twelve small pictures, one for each of four exit dates and three volatility regimes, each carrying a stated probability measure and three numbers. Section~3 explains why it is a grid and not a slider.

\begin{table}[htbp]
\centering\small
\caption{Round-trip friction on the Figure~2 unwind. Spread is a fraction of mid; the round trip is half the spread on the 7.02 entry plus half on the 11.46 exit; the share is of the 4.21 gross floor of the 45\% curve; the net figure is the 4.44 gross \PnL{} at unchanged spot less the round trip.}
\begin{tabular}{@{}lrrr@{}}
\toprule
Spread (fraction of mid) & Round-trip cost & Cost as share of the 4.21 gross floor & Net \PnL{} at unchanged spot (4.44 less cost) \\
\midrule
...
Gresh, Rogowitz, Tignor and Mayland (1999) rendered option value over the tenor as interactive 3D surfaces; Fei and Prakash (2001) gave profit bands their own planes in a volume rendering. Both reached for depth to hold spot, time and volatility. Munzner (2014) ranks depth among the weakest visual channels, for occlusion, ambiguity and the cost of rotating to compare; her rule, \emph{no unjustified 3D}, was written for this case, Ko et al.\ (2016) record the same complaints from finance, and none of the tools in Table~3 renders payoff surfaces in 3D. The planar alternative is old and better: \textbf{small multiples} (Tufte, 1983), a grid of miniature charts on common scales, indexed by the variables one would otherwise have encoded in depth.

\subsection{The uncertainty literature}

Small multiples solve time and volatility, not the measure, and the recent literature is uncomfortable reading for anyone proposing shaded bands. Correll and Gleicher (2014) showed that error bars are misread even by trained readers; Hullman, Resnick and Adar (2015) that animated hypothetical outcome plots outperform static intervals; Fernandes, Walls, Munson, Hullman and Kay (2018), with transit riders in the field, that quantile dotplots beat shaded intervals, most of all for the least expert. Joslyn and Savelli (2021) name the failure, the \emph{deterministic construal error}, in which a non-expert reads the edge of a band as a boundary and its interior as a certainty, and Padilla, Kay and Hullman (2021) conclude that frequency framing is the robust choice for non-experts. Section~4 therefore offers both encodings and says which is for whom.

\subsection{The brokers}

Table~3 records what exists as at September 2026. Interactive Brokers' Option Analytics bends a value curve by ``changing volatility assumptions, stepping closer to expiration and adjusting the price of the underlying'': omission one repaired, one scenario at a time. Its web order ticket, seen on 4 September 2026 for a zero-day Apple call, is the point of sale itself: maximum loss, break-even and maximum return, a P\&L line against price at a selectable date, a spot-shock ladder from $-30\%$ to $+10\%$ with Greeks, no volatility control, and a ``market implied probability of profit'' of 9.5\%, at expiry. Its Probability Lab reports a ``percentage likelihood of profit'' against a market-implied distribution: omission two repaired, but only ``at the time the option expires''. thinkorswim quotes a probability of expiring and an expected move, draws a probability-of-expiring cone on its price charts, and bends its risk profile under slider control; Power E*TRADE's Snapshot Analysis prints a probability of profit against a risk graph drawn today and at expiry, and its TradeLab scrolls a timeline of what-if values under a chosen implied-vol forecast. Beneath the brokers, OptionStrat lays out ``the expected profit and loss of your trade at various prices and dates'' on one fixed table with an implied-vol slider, and OptionsProfitCalculator.com does the same at a fixed vol, defining its probability of profit as breakeven or better ``on the day of expiry''. The calculators have already fixed the time axis; what they have not fixed is what the rest of this article is about.
...
Power E*TRADE Snapshot Analysis & Today and expiry & Expiry-only number & One at a time \\
Power E*TRADE TradeLab & Yes, by timeline scroll & Expiry-only probabilities & One at a time; vol by menu \\
...
\item \textbf{Columns:} three implied-volatility regimes $\{\sigma_{\text{low}}, \sigma_0, \sigma_{\text{high}}\}$, user-set, each a representative point for a bucket (below 20\%, 20\% to 35\%, above 35\% in the base case). The columns are scenarios, not forecasts; the probability printed in a cell is the model-implied probability of the bucket. ``Regime'' is used throughout for that bucket and for nothing more: a range of implied vol at the exit date, not a persistent economic state.
...
\item \textbf{The measure, restored twice.} Within a cell, the 68\% and 95\% bands of $S_u$ \emph{conditional on the regime}; across cells, each regime's probability, so that three columns are never weighted equally when the market does not. Both follow from one small joint model, an assumption rather than a derivation: $\ln\sigma_u$ normal with vol-of-vol $\nu$, $\ln S_u$ lognormal and correlated with it at $\rho$. Conditional on the regime the band's centre shifts by $\rho\,\sigma_R\sqrt{u}\,z$, $z$ being the standardised vol move, and its variance shrinks to $(1-\rho^2)\sigma_R^2 u$; $\rho = 0$ recovers the independent grid (Appendix~A). These are model-implied probabilities under the risk-neutral drift $\mu = r$, not forecasts of real-world frequency; a physical drift and its calibration would be needed before the display supported likelihood claims (Section~5).
\item \textbf{The question answered.} Each cell prints $P(\Pi > 0)$, $E[\Pi]$ and the 5\% \PnL{} quantile $Q_{5\%}(\Pi)$, a downside-tail statistic that is a gain where the whole curve sits above zero, so the reader is not asked to integrate a curve against a band by eye, the one operation the structurer can do in their head and the retail user cannot. Probability of profit and expected \PnL{} summarise the centre of the distribution; risk lives in its left tail and in what the model omits, from expected shortfall and drawdown to the margin call and liquidity. The grid carries the first on its face and shows the rest as geometry; two centre statistics are not a risk measure and are not called one.
\end{itemize}

Every element of the display, and every number in this article, carries one of four labels. \emph{Scenario}: a deterministic repricing under stated inputs, which is what the curves are. \emph{Model-implied}: a probability or expectation under the joint model of Appendix~A, which is what the bands, the regime probabilities and the cell statistics are. \emph{Historical}: an estimate from data, which is what $\rho$ and $\nu$ are. \emph{Market-implied}: read from traded prices, which is what Apple's implied vol on the day of writing is. Figure captions state the label, and a number in the text without one is model-implied.
...
Section~3 flagged that shaded bands are the encoding the literature predicts to underperform for this audience; the resolution is two modes of one specification. \emph{Expert mode} is Figure~4: continuous bands that compose with the curve and numerical summaries in every cell, for the reader who already carries the surface in their head. \emph{Retail mode} replaces bands with frequency-framed dots and counts; it is the point-of-sale display and the one Section~8 tests. \textbf{Figure~5} shows the three-months-in row in retail mode: twenty dots, each an equally likely share price under the cell's measure, coloured by whether the position unwinds at a profit there. The probability of profit is a count, 3 of 20, 4 of 20, 20 of 20, and the deterministic construal error has no edge to attach to. Nothing in the specification changes if the dots win, as the literature predicts; whether they do for this problem is an open question until Section~8 is run.

\emph{What a broker would display.} At the point of sale the default is one row, not twelve: the three cells for the user's stated exit date, or the midpoint of the tenor if none is given, in retail mode, each headed by a sentence of the form ``if implied volatility is around 45\% in three months, this trade shows a profit in 20 of 20 likely outcomes; the model gives that regime about a one-in-five chance''. Beneath the row sit the two disclosure lines of Section~7 and one control that expands it to the full grid. The expert grid is the second screen, and the staging is itself a choice the study can test.
...
The 0DTE example is a boundary test of the display architecture, not a calibrated intraday model. \textbf{Figure~7} rebuilds the grid for an at-the-money straddle on an index at 100, bought at the open at 18\% for $0.905$ points, rows at one, three and five hours and the close, columns at 12\%, 18\% and 30\%. The joint model does not transfer: intraday vol moves are jumps, and a lognormal vol-of-vol prices a 12-to-30 move within three hours as a ten-sigma event, so the grid is drawn with $\rho = 0$, without regime probabilities, and says so. What survives: expected \PnL{} is constant down each column, $-0.30$, $0.00$, $+0.60$, because without drift or correlation the expected unwind value does not depend on when it happens, whilst the probability of profit rises through the day in the low- and base-vol columns (1\% to 23\%, 32\% to 42\%) and falls in the high-vol column (100\% to 63\%) as the vega cushion decays, so the grid can say \emph{when} the trade is best unwound in each regime. What does not survive, and would need a jump model rather than a diffusion: jumps and event clustering in spot and vol, intraday seasonality against an annualised implied number, execution latency and a spread that is a large fraction of a sub-point premium, and settlement on the final print. Halts and early exercise do not arise for cash-settled European index options.
...
\subsection{Against the Greeks}

A quant's first reaction is that the Greeks already say all this, and to a quant they do. The division of labour is simple: a Greek is a local partial derivative at $(S_0, 0, \sigma_0)$, the grid a set of finite displacements in $(S, u, \sigma)$ under a stated measure, and the Greeks compose into a picture only in a head that can multiply a vega by a scenario whilst holding the theta, which describes the structurer and not the cohort de Silva, Smith and So measured. The Greeks carry no measure at all; the grid carries one in every cell.

\section{Limitations: what is arithmetic and what is model}

\noindent The display has four layers, and they deserve unequal trust. The curves are arithmetic: Black--Scholes repricings under stated inputs, wrong only to the extent that a flat vol shift misdescribes the smile. The band widths rest on $\sigma_R$ and on lognormality, assumptions every options desk makes daily. The regime probabilities rest on the vol law, and the lognormal law overstates the tails against a mean-reverting one, 36\%, 36\% and 29\% at six months against 16\%, 76\% and 7\% (Appendix~A). The conditional shift is the least robust element: it rests on a linear-Gaussian conditioning, on few extreme observations, and on a parameter whose sign is an input and not a law, strongly negative for Apple and Goldman Sachs, weakly negative for Amazon and Alphabet, absent for IBM and positive for gold (Table~7). \textbf{Table~4} reruns the bottom row of Figure~4 changing one thing at a time. The ordering of the three columns survives every row; the levels move by a few points under the parameter, drift, spread and cross-underlying changes, and by much more when the mean-reverting law is applied to the shift, which is why Appendix~A uses that law for the column weights and not for the shift. That hybrid is not proposed as a pricing model; it is a transitional implementation whose job is to show how the display should expose separate sources of uncertainty, and a single coherent joint process is the advanced column of Table~5.

\begin{table}[htbp]
\centering\small
\caption{Robustness of the bottom row (at expiry) of Figure~4 to the modelling choices, one change at a time from the base case. Regime probability, probability of an unwind at a profit and expected \PnL{} per share, by column. Cross-underlying parameters are the monthly estimates of Table~7. \emph{Status: model-implied throughout; the spread row nets a 10\% proportional bid--ask spread, half on entry and half on exit.}}
\begin{tabular}{@{}L{4.4cm}rrrrrrrrr@{}}
\toprule
& \multicolumn{3}{c}{$P(\text{regime})$} & \multicolumn{3}{c}{$P(\Pi>0)$} & \multicolumn{3}{c}{$E[\Pi]$} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}
Change from base case & 15\% & 25\% & 45\% & 15\% & 25\% & 45\% & 15\% & 25\% & 45\% \\
\midrule
None: lognormal vol, $\rho=-0.6$, $\nu=0.85$, $\mu=r$, mid-market & 36\% & 36\% & 29\% & 23\% & 32\% & 69\% & $-2.31$ & $-1.31$ & $+4.58$ \\
Independent columns, $\rho = 0$ & 36\% & 36\% & 29\% & 19\% & 43\% & 66\% & $-2.67$ & $+0.14$ & $+5.78$ \\
Mean-reverting vol law, $\kappa = 6.9$ & 16\% & 76\% & 7\% & 62\% & 32\% & 93\% & $+1.77$ & $-1.31$ & $+12.53$ \\
Physical drift, $\mu = 10\%$ & 36\% & 36\% & 29\% & 35\% & 34\% & 66\% & $-1.15$ & $-0.97$ & $+4.06$ \\
Round-trip spread of 10\% of mid & 36\% & 36\% & 29\% & 19\% & 27\% & 65\% & $-2.90$ & $-1.95$ & $+3.65$ \\
Goldman Sachs parameters, $\rho=-0.59$, $\nu=0.73$ & 33\% & 41\% & 26\% & 26\% & 32\% & 71\% & $-2.01$ & $-1.26$ & $+5.16$ \\
Alphabet parameters, $\rho=-0.28$, $\nu=0.88$ & 36\% & 35\% & 29\% & 21\% & 41\% & 66\% & $-2.49$ & $-0.15$ & $+5.01$ \\
Gold ETF parameters, $\rho=+0.48$, $\nu=0.52$ & 27\% & 55\% & 18\% & 18\% & 36\% & 71\% & $-2.85$ & $-0.75$ & $+10.48$ \\
\bottomrule
\end{tabular}
\end{table}

Four further limits. The bands are drawn under $\mu = r$ for consistency with the pricing model; for likelihoods the physical drift is the defensible choice, the user should supply it, and the display must not choose silently: $\mu$ from 0 to 15\% moves the base column's probability of profit at expiry from 32\% to 37\%. Flat repricings are a scenario, not a calibration; the natural extension prices each cell off a smile-consistent shift under Heston or SABR dynamics. The curves are mid-market, and a grid that shows net-of-spread \PnL{} is the only version a regulator could sensibly mandate. And the claim that any of this improves comprehension rests on the uncertainty literature, not yet on readers of option displays; until Section~8 is run the grid is a proposal with its assumptions on its face, not a demonstrated improvement on the brokers' tools.

\section{Disclosure precedent: what PRIIPs got wrong}

\noindent The last time a probabilistic point-of-sale display was mandated, it failed. The EU's PRIIPs Key Information Document, from 2018, carried four performance scenarios computed from the product's own recent returns, procyclical by construction; after a long bull market the FCA's feedback statement recorded ``significant concerns'' that they produced ``misleading illustrations across almost all asset classes'' (FCA, 2019), and PS22/2 (FCA, 2022) removed them from 25 March 2022 in favour of ``narrative information on performance''. The UK is now replacing PRIIPs wholesale: the Consumer Composite Investments framework (PS25/20; FCA, 2025) commences on 6 April 2026 with a transition to 8 June 2027, retains standardised content on costs, risk and return and past performance, and gives manufacturers ``considerable freedom over the design of product summaries''. An exchange-traded option sold to a retail client will ordinarily fall within the new perimeter (FCA, 2026; SI 2024/1198); which firm in the chain owes the product summary is a separate question. The regulator has moved from prescribing a probabilistic picture, to withdrawing it, to prescribing content and leaving design free: the gap a display standard would fit, and the gap the PRIIPs scenarios fell through. Three things separate the proposal from the precedent: the PRIIPs measure came from the product's own past returns, the grid's from a stated model the user can change; the PRIIPs table hid its model, the grid prints its assumptions on its face; and the PRIIPs scenarios were never tested on readers before they were mandated.

\section{Implementation}

\noindent Table~5 is the specification a broker or risk platform would build from; the minimum column is modest work by the standards of what the platforms ship. Every rendering carries two disclosure lines, the status label of each element under the key of Section~4 and the spread assumption in force, and falls back to the static diagram with a notice when data are unavailable.
...
Layout & One row at the stated exit date in retail mode; full grid on demand & Expert grid by default for professional clients; user-defined rows \\
...
\emph{Calibration across underlyings.} Table~7 applies the same estimator to six other pairs with a free Cboe implied-volatility index, using closes from Nasdaq sampled every fifth trading day (253 observations per name, 1 September 2021 to 3 September 2026), with weekly changes between consecutive observations and monthly changes between every fourth, both non-overlapping; the Apple row is resampled the same way for comparability and so differs slightly from the daily-derived figures above. Vol-of-vol is stable across names, $0.5$ to $1.2$ at the monthly horizon; the correlation is not. It is strongly negative for Apple and Goldman Sachs, weakly negative for Amazon and Alphabet, absent for IBM, and positive for gold, whose implied vol rises when the metal rallies, with oil in between. The sign of the conditional shift is therefore an input per underlying and never a default, and a display that took Apple's $-0.6$ as a constant would shift gold's bands the wrong way. The regime buckets of Section~4 are Apple's; the last column shows how differently the same three buckets would weight for a name that lives above 35\% (Amazon, oil) or below 20\% (gold), which is why the levels are user-set. The S\&P~500 against VIX is the obvious further row and was not run for this draft; rate and credit underlyings are outside the table for want of a comparable free index history.

\begin{table}[htbp]
\centering\small
\caption{Spot--vol correlation $\rho$ and vol-of-vol $\nu$ across underlyings, September 2021 to September 2026, from Cboe implied-volatility indices and Nasdaq closes sampled every fifth trading day. Weekly: 251 to 252 non-overlapping changes; monthly: 62 to 63. Median index level and share of trading days in each of the display's three buckets, from the full daily index series. \emph{Status: historical.}}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
& \multicolumn{2}{c}{Weekly} & \multicolumn{2}{c}{Monthly} & & Days below 20 / \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Underlying / index & $\rho$ & $\nu$ & $\rho$ & $\nu$ & Median vol & 20 to 35 / above 35 \\
\midrule
Apple / VXAPL & $-0.53$ & 0.87 & $-0.52$ & 0.75 & 29\% & 4 / 80 / 15 \\
Amazon / VXAZN & $-0.37$ & 1.02 & $-0.34$ & 0.96 & 36\% & 0 / 45 / 55 \\
Alphabet / VXGOG & $-0.32$ & 0.94 & $-0.28$ & 0.88 & 33\% & 0 / 60 / 40 \\
Goldman Sachs / VXGS & $-0.57$ & 0.90 & $-0.59$ & 0.73 & 30\% & 2 / 76 / 22 \\
IBM / VXIBM & $-0.03$ & 1.17 & $-0.04$ & 1.22 & 26\% & 15 / 63 / 22 \\
Gold ETF (GLD) / GVZ & $+0.31$ & 0.73 & $+0.48$ & 0.52 & 17\% & 74 / 24 / 2 \\
Oil ETF (USO) / OVX & $+0.06$ & 0.90 & $+0.17$ & 0.71 & 38\% & 0 / 33 / 67 \\
\bottomrule
\end{tabular}
\end{table}

\emph{Drift.} The bands and statistics use $\mu = r = 4\%$. Under $\mu$ of 0\%, 4\%, 10\% and 15\% the probability of profit at expiry in the 25\% column is 31.8\%, 32.0\%, 34.1\% and 37.4\%, and the expected \PnL{} $-1.39$, $-1.31$, $-0.97$ and $-0.49$; the drift is a user input in Table~5.
...
\emph{Reproduction.} All figures and every number quoted in the text are generated by Python (NumPy, SciPy, Matplotlib): the model in one file, the figures in a second, the calibrations and Tables~4 and~7 in two more, typeset through pdfLaTeX so that figure text and body text share a face; a validation script prints the quoted numbers and the Monte Carlo check. The Cboe index series are archived with access dates; the Nasdaq closes are fetched by script and not redistributed. The package is at github.com/phykit/payoff-diagram (Kittson, 2026), with a one-command build.

\section*{References}
\sloppy
\begin{list}{}{\leftmargin=1.4em \itemindent=-1.4em \itemsep=4pt \topsep=6pt}
\item Bauer, R., Cosemans, M. and Eichholtz, P. (2009). Option trading and individual investor performance. \emph{Journal of Banking \& Finance}, 33(4), 731--746. doi:10.1016/j.jbankfin.2008.11.005.
\item Bogousslavsky, V. and Muravyev, D. (2024). An anatomy of retail option trading. Working paper, SSRN 4682388.
\item Brown, A. (2022). Should individual investors trade options? \emph{Wilmott}, 2022(120). doi:10.54946/wilm.11024.
\item Charles Schwab (2026). \emph{Using options volatility data to research moves}. schwab.com/learn/story/using-sentiment-analysis-tools-your-trading, accessed 4 September 2026.
\item Cboe Global Markets (2026a). \emph{Cboe Global Markets reports trading volume for July 2026}. Press release, 5 August 2026. \url{ir.cboe.com/news/news-details/2026/Cboe-Global-Markets-Reports-Trading-Volume-for-July-2026/default.aspx}, accessed 4 September 2026.
\item Cboe Global Markets (2026b). \emph{Cboe Equity VIX on Apple (VXAPL), daily history}, and the corresponding VXAZN, VXGOG, VXGS, VXIBM, GVZ and OVX histories. \url{cdn.cboe.com/api/global/us_indices/daily_prices/VXAPL_History.csv} and sibling files, accessed 4 September 2026.
\item Cboe Global Markets (2026c). \emph{State of the options industry: options market continued to break records in Q2 2026}. Cboe Insights, \url{cboe.com/insights/posts/state-of-the-options-industry-options-market-continued-to-break-records-in-q-2-2026}, accessed 4 September 2026.
\item Correll, M. and Gleicher, M. (2014). Error bars considered harmful: exploring alternate encodings for mean and error. \emph{IEEE Transactions on Visualization and Computer Graphics}, 20(12), 2142--2151. doi:10.1109/TVCG.2014.2346298.
...
\item Fei, T.T. and Prakash, E.C. (2001). Volume visualization of payoff regions for derivatives risk management. \emph{Journal of Visualization}, 4(4), 383--390. doi:10.1007/BF03183901.
\item Fernandes, M., Walls, L., Munson, S., Hullman, J. and Kay, M. (2018). Uncertainty displays using quantile dotplots or CDFs improve transit decision-making. \emph{Proceedings of the 2018 CHI Conference on Human Factors in Computing Systems}, paper 144, 1--12. doi:10.1145/3173574.3173718.
...
\item Gresh, D.L., Rogowitz, B.E., Tignor, M. and Mayland, E. (1999). An interactive framework for visualizing foreign currency exchange options. \emph{Proceedings of IEEE Visualization '99}, 453--456. doi:10.1109/VISUAL.1999.809929.
\item Hullman, J., Resnick, P. and Adar, E. (2015). Hypothetical outcome plots outperform error bars and violin plots for inferences about reliability of variable ordering. \emph{PLoS ONE}, 10(11), e0142444. doi:10.1371/journal.pone.0142444.
...
\item Interactive Brokers (2026b). \emph{Probability Lab}. interactivebrokers.co.uk/en/?f=40653, accessed 4 September 2026.
\item Israelov, R. and Nielsen, L.N. (2014). Covered call strategies: one fact and eight myths. \emph{Financial Analysts Journal}, 70(6), 23--31. doi:10.2469/faj.v70.n6.3.
\item Joslyn, S. and Savelli, S. (2021). Visualizing uncertainty for non-expert end users: the challenge of the deterministic construal error. \emph{Frontiers in Computer Science}, 2, 590232. doi:10.3389/fcomp.2020.590232.
\item Kittson, S. (2026). \emph{payoff-diagram: reproduction package for ``One Trade, Twelve Honest Pictures''}. github.com/phykit/payoff-diagram, MIT licence, accessed 4 September 2026.
\item Ko, S., Cho, I., Afzal, S., Yau, C., Chae, J., Malik, A., Beck, K., Jang, Y., Ribarsky, W. and Ebert, D.S. (2016). A survey on visual analysis approaches for financial data. \emph{Computer Graphics Forum}, 35(3), 599--617. doi:10.1111/cgf.12931.
\item Munzner, T. (2014). \emph{Visualization Analysis and Design}. AK Peters/CRC Press.
\item Nasdaq (2026). \emph{Historical data: AAPL, AMZN, GOOG, GS, IBM, GLD, USO}. nasdaq.com/market-activity/stocks/aapl/historical and sibling pages, accessed 4 September 2026; not redistributed.
...
\item Padilla, L., Kay, M. and Hullman, J. (2021). Uncertainty visualization. In \emph{Wiley StatsRef: Statistics Reference Online}, 1--18. Wiley. doi:10.1002/9781118445112.stat08296.
\item The Consumer Composite Investments (Designated Activities) Regulations 2024, SI 2024/1198, regulation 4. legislation.gov.uk/uksi/2024/1198.
\item Tufte, E.R. (1983). \emph{The Visual Display of Quantitative Information}. Graphics Press.
\item Wystup, U. (2019). Shedding light on common misconceptions. \emph{Wilmott}, 2019(104), 16--19. doi:10.1002/wilm.10801. Preprint at mathfinance.com/publications.
\end{list}

\vspace{8pt}
\noindent\rule{\linewidth}{0.4pt}\\[4pt]
{\small\itshape Simon Kittson is a solicitor (England \& Wales) and CFA charterholder specialising in derivatives and collateral. This article develops his 2018 MSc project at City, University of London (now City St George's), supervised by Jason Dykes. linkedin.com/in/kittson}
